...
function cargarUsuarios() {
    ...
    fetch('../crud/crudDirector.php', {
        method: 'POST',
        body: new URLSearchParams({accion: 'listar'})
    })
    .then(r=>r.json())
    .then(data=>{
        let output = data.filter(u => u.rol_id != 4 && u.rol_id != 5);
        output.sort((a, b) => (a.rol > b.rol) ? 1 : (a.rol < b.rol) ? -1 : 0);
        ...
        <tr><th>Nombre</th><th>Email</th><th>Rol</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>`;
        output.forEach(u=>{
            let acciones = `
                <button class="btn btn-sm btn-outline-secondary" onclick="formUsuario(${u.id})" title="Editar"><i class="bi bi-pencil"></i></button>
...
function formUsuario(id) {
    fetch('../crud/crudDirector.php', { method: 'POST', body: new URLSearchParams({accion: 'roles'}) })
    .then(r => r.json())
    .then(rolesArr => {
        if (!id) {
            // NUEVO USUARIO
            let html = `<form id="formNuevoUsuario" class="p-4" style="min-width:320px;max-width:400px;">
                ...
                <div class="mb-2">
                    ...
                    <select name="rol_id" class="form-select" required>
                        ...
                        ${rolesArr.filter(r=>r.id!=4&&r.id!=5).map(r=>`<option value="${r.id}">${r.nombre}</option>`).join("")}
                    </select>
                </div>
            ...
            document.getElementById("formNuevoUsuario").onsubmit = function(e){
                ...
                fetch('../crud/crudDirector.php',{method:'POST',body:form})
            ...
            // EDITAR USUARIO
            fetch('../crud/crudDirector.php', { method: 'POST', body: new URLSearchParams({accion:'listar'}) })
            .then(r=>r.json()).then(users=>{
                ...
                let html = `<form id="formEditUsuario" class="p-4" style="min-width:320px;max-width:400px;">
                    ...
                    <div class="mb-2">
                        ...
                        <select name="rol_id" class="form-select" required>
                            <option value="">Seleccione un rol</option>
                            ${rolesArr.filter(r=>r.id!=4&&r.id!=5).map(r=>`<option value="${r.id}" ${u.rol_id==r.id?'selected':''}>${r.nombre}</option>`).join("")}
                        </select>
                    </div>
                    <div class="text-end mt-3">
...
function cargarCoordinadores() {
    ...
    .then(r=>r.json()).then(data=>{
        ...
        <tr><th>Nombre</th><th>Email</th><th>Rol</th><th>Estado</th></tr></thead><tbody>`;
        output.forEach(u=>{
            html += `<tr>
                <td>${u.nombre}</td>
                <td>${u.email}</td>
...
function cargarProfesores() {
    const cont = document.getElementById('tabla-profesores');
    fetch('../crud/crudDirector.php', {
        method: 'POST',
        body: new URLSearchParams({accion: 'listar'})
    })
    .then(r=>r.json()).then(data=>{
        let output = data.filter(u=>u.rol_id==2);
        if (output.length === 0) {
            cont.innerHTML = `<div class="alert alert-info">No hay profesores registrados.</div>`;
            return;
        }
        let html = `<table class="table table-bordered align-middle"><thead>
        <tr><th>Nombre</th><th>Email</th><th>Rol</th><th>Estado</th></tr></thead><tbody>`;
...
function cargarClases() {
    ...
    .then(r=>r.json()).then(data=>{
        let html = `<button class="btn btn-dark btn-sm mb-2" onclick="formClase()">Nueva Clase</button>
        <table class="table table-striped align-middle"><thead>
        <tr><th>Nombre</th><th>Profesor</th><th>Periodo</th><th>Acciones</th></tr></thead><tbody>`;
        data.forEach(c=>{
            html += `<tr>
                <td>${c.nombre}</td>
                <td>${c.profesor}</td>
                <td>${c.periodo}</td>
                <td>
                    <button class="btn btn-sm btn-outline-secondary" onclick="formClase(${c.id})" title="Editar"><i class="bi bi-pencil"></i></button>
...
function formClase(id){
    // Obtén la lista de profesores para el select
    fetch('../crud/crudDirector.php', { method: 'POST', body: new URLSearchParams({accion: 'listar'}) })
    .then(r=>r.json())
    .then(usuarios=>{
...
function cargarReportes() {
    ...
    .then(r=>r.json()).then(data=>{
        let html = `<table class="table table-bordered align-middle"><thead>
        <tr><th>ID</th><th>Alumno</th><th>Comentario</th><th>Fecha</th></tr></thead><tbody>`;
        data.forEach(r=>{
            html += `<tr>
                <td>${r.id}</td>
                <td>${r.alumno}</td>
...
function cargarRecursos() {
    const cont = document.getElementById('tabla-recursos');
    fetch('../crud/crudRecursos.php', {
        method: 'POST',
        body: new URLSearchParams({accion: 'listar'})
    })
    .then(r=>r.json()).then(data=>{
        if (!Array.isArray(data) || data.length === 0) {
            cont.innerHTML = `<div class="alert alert-info">No hay recursos registrados.</div>`;
            return;
        }
        let html = `<table class="table table-bordered align-middle"><thead>
        <tr><th>Título</th><th>Archivo</th><th>Clase</th><th>Fecha subida</th><th>Acciones</th></thead><tbody>`;
        data.forEach(m=>{
            let acciones = `<button class="btn btn-sm btn-outline-danger" onclick="eliminarRecurso(${m.id})" title="Eliminar"><i class="bi bi-trash"></i></button>`;
            html += `<tr>
                <td>${m.titulo}</td>
                <td>${m.archivo}</td>
                <td>${m.clase}</td>
                <td>${m.fecha_subida}</td>
                <td>${acciones}</td>
            </tr>`;
        });
        html += `</tbody></table>`;
...
function cargarAprobaciones() {
    ...
    .then(r=>r.json()).then(data=>{
        let html = `<table class="table table-bordered align-middle"><thead>
        <tr><th>Alumno</th><th>Clase</th><th>Acciones</th></tr></thead><tbody>`;
        data.forEach(i=>{
            html += `<tr>
                <td>${i.alumno}</td>
                <td>${i.clase}</td>
